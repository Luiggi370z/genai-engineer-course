...
export const foundations: PhaseContent = {
  id: "p1",
  weeks: "Weeks 1–2",
  accent: { light: "#0A714E", dark: "#1AAD7B" },
  ...
  objectives: [
    ...
    {
      id: "p1-o4",
      text: "**Constrain** a model’s output with a schema so answers parse on the first try, then compare hosted vs. local on both numbers that matter — violation rate and per-field accuracy against labelled answers",
    },
    {
      id: "p1-o5",
...
cos(v[0], v[2])   # ~0.1  unrelated`,
  ...
  exercises: [
    ...
    },
    {
      ...
      effort: { fast: 40, integration: null, realistic: 65 },
      rung: "faded",
      proves: "integrate",
      task: "Pull {vendor, date, total} from 20 messy invoices using structured outputs on (a) a frontier model and (b) qwen3.5:9b locally. Report both numbers per provider: the schema-violation rate, and field accuracy against your own labelled answers — `compare_providers` takes the clients as arguments so the same procedure runs offline against fakes.",
      assesses: ["p1-o4"],
      solution: [
        "Both should hit ~0 schema violations (constrained decoding does that for free), which is exactly why the violation rate alone cannot decide the shoot-out.",
        "Accuracy is per field, not averaged: `vendor` is copied, `total` has to come off the right line, and `date` has to be reformatted — that last column is where the small model usually loses, and an average would hide it.",
        "A provider that returns nothing for a row scores that row wrong rather than being dropped from the denominator. Shortening the list slides every later answer onto the wrong expected row.",
        "That per-field number is your evidence for model tiering in Phase 5. Keep it.",
      ],
    },
    {
      id: "p1-e4",
