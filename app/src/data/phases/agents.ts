...
result = agent.run_sync("Email the team the Q3 recap")  # typed result out`,
        },
        {
          ...
          text: "Write the loop from scratch once (you just did). Then default to **LangGraph** for anything real — durability and HITL are worth the graph. Reach for **Pydantic AI** when a single well-typed agent is all you need, and **CrewAI** when you’re sketching a multi-role team and want speed over control. Your Phase-1 provider-agnostic client sits underneath all three.",
        },
      ],
    },
  ],
  ...
  exercises: [
    {
      ...
      rung: "faded",
      task: "Build a ReAct agent with two tools (a calculator and a web search) and a hard step cap + wall-clock timeout. Run it on a frontier model AND on qwen3.5:8b locally; document where local tool-calling wobbles and add validate-and-retry.",
    ...
    },
    {
      ...
      assesses: ["p3-o2"],
      solution: [
        "The gated tool should refuse to execute until an approval flag is passed — your first taste of HITL wiring.",
    ...
    },
    {
      ...
      rung: "faded",
      task: 'Build the same small "answer-with-tools" agent in LangGraph, Pydantic AI, and CrewAI. Write a one-paragraph verdict: which felt right, and for what job.',
      assesses: ["p3-o4"],
      solution: [
        "You’ll feel the trade directly: LangGraph’s ceremony buys durability; Pydantic AI’s types buy safety with less code; CrewAI’s roles buy speed at the cost of fine control.",
        "There is no winner — the deliverable is your ability to justify a choice out loud, which is exactly what interviews probe.",
      ],
    },
    {
      id: "p3-e5",
      ...
      solution: [
        ...
        "The impossible goal terminates and reports failure rather than looping until the cap. An agent that hits `max_steps` on every hard task is technically contained and practically useless — you want it to notice.",
...
def schedule_event(title: str, start_iso: str, duration_min: int) -> str:
    ...
    deliverables: [
      ...
      {
        id: "w2-d6",
        text: "`docker compose up` runs it; all credentials come from **env vars**",
      },
    ],
    stretch: [
  ...
  checkpoint: [
    ...
    },
    {
      ...
      a: "When a single, well-typed agent with tools is all you need and a state graph would be overkill. LangGraph earns its ceremony when you need durability, branching, cycles, or HITL that pauses for a long time; Pydantic AI wins on minimal, type-safe simplicity.",
    },
    {
      id: "p3-q5",
