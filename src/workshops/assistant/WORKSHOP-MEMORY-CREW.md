...
```
turn ──► classify ──► write(kind, text, source=…, ttl=…) ──► AssistantMemory
                                                              │  four kinds
...
Implement `memory.py` and `crew.py`. Reference and tests:
`after/src/assistant/memory.py`, `after/src/assistant/crew.py`,
`after/tests/test_memory.py`, `after/tests/test_crew.py`.
