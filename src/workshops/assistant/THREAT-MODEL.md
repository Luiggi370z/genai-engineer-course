...
| Tampering — content claims "already approved" | agent loop | approval lives only in server-side grants, never in content; each grant pays for ONE run | `test_an_approval_authorizes_exactly_one_run`; phase6 `approval-bypass` rows |
...
| **E**levation of privilege — agent fires an irreversible tool | agent loop | HITL: `requires_approval` tools pause; approval is a counted grant consumed per run; replayed `/approve` (idempotency key) is a no-op | `test_a_gated_tool_pauses_until_it_is_approved`, `test_a_replayed_approval_does_not_grant_twice` |
