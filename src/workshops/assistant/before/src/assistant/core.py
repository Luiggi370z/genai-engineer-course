...
class Assistant:
    ...
    def _usage_attributes(self, used: Usage, tier: str) -> dict[str, Any]:
        ...
        TOKENS_IN, TOKENS_OUT, TOKENS_TOTAL and COST (`used.cost(tier)`). They go
        on the compose span rather than only on the root because a request can
        compose more than once: summing children is arithmetic, splitting a total
    ...
    def ask_stream(self, question: str, subject: str = auth.ANONYMOUS) -> Iterator[str]:
        ...
        with observe.streaming_root(
            ...
            for kind, chunk in gated_chunks(produced, self.settings.stream_mode):
                ...
                # reason on the root span as observe.ABANDONED.
                #
